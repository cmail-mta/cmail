...
int sasl_challenge(SASL_CONTEXT* ctx, char** response){
	switch(ctx->method){
		case SASL_PLAIN:
			*response = NULL;
			return SASL_CONTINUE;
		default:
			logprintf(LOG_ERROR, "No challenge handler defined for SASL method\n");
...
int sasl_continue(SASL_CONTEXT* ctx, char* data, char** response){
	int length, off_user = 0, off_pass = 0;

	if(!response){
		logprintf(LOG_ERROR, "NULL response pointer in sasl_continue\n");
		return SASL_ERROR_PROCESSING;
	}
	...
	switch(ctx->method){
		case SASL_PLAIN:
			...
			length = auth_base64decode(data) + 1;
			if(length < 4){ //2x NUL, 2x min 1 byte
				logprintf(LOG_ERROR, "Failed to decode PLAIN authentication parameter\n");
				return SASL_ERROR_DATA;
			}
			...
			*response = data + off_pass;
			return SASL_DATA_OK;
		default:
			logprintf(LOG_ERROR, "No continuation handler defined for SASL method\n");
			return SASL_ERROR_PROCESSING;
	}
...
int sasl_begin(SASL_CONTEXT* ctx, SASL_USER* user, char* method, char* data, char** response){
	//sanity check
	...
	if(data){
		return sasl_continue(ctx, data, response);
	}

	return sasl_challenge(ctx, response);
}
