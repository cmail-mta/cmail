int client_line(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;

	logprintf(LOG_ALL_IO, ">> %s\n", client_data->recv_buffer);
	switch(client_data->state){
		case STATE_NEW:
			return smtpstate_new(client, database, path_pool);
		case STATE_IDLE:
			return smtpstate_idle(client, database, path_pool);
		case STATE_AUTH:
			return smtpstate_auth(client, database, path_pool);
		case STATE_RECIPIENTS:
			return smtpstate_recipients(client, database, path_pool);
		case STATE_DATA:
			return smtpstate_data(client, database, path_pool);
		default:
...
int client_free(CONNECTION* client){
	CLIENT* client_data = (CLIENT*)client->aux_data;

	logprintf(LOG_DEBUG, "Freeing client data\n");
...
int client_memtimeout(CONNECTION* client){
	CLIENT* client_data = (CLIENT*)client->aux_data;
...
int client_resolve(CONNECTION* client){
	...
	if(getpeername(client->fd, (struct sockaddr*)&data, &len) < 0){
		logprintf(LOG_ERROR, "Failed to get peer name: %s\n", strerror(errno));
		return -1;
	}

	status=getnameinfo((struct sockaddr*)&data, len, client_data->peer_name, MAX_FQDN_LENGTH - 1, NULL, 0, 0);
	if(status){
		logprintf(LOG_WARNING, "Failed to resolve peer: %s\n", gai_strerror(status));
		return -1;
	}

	logprintf(LOG_INFO, "Connection from %s\n", client_data->peer_name);
...
int client_accept(DATABASE* database, CONNECTION* listener, CONNPOOL* clients){
	...
	*actual_data = empty_data;
	...
	if(client_resolve(&(clients->conns[client_slot])) < 0){
		logprintf(LOG_WARNING, "Peer resolution failed\n");
	...
	if(listener_data->fixed_user){
		...
		if(!actual_data->sasl_user.authenticated || !actual_data->sasl_user.authorized){
			...
			//query routing data
			actual_data->originating_route = route_query(database, actual_data->sasl_user.authorized);
		}
	}

	actual_data->current_mail.submitter = actual_data->peer_name;
	logprintf(LOG_DEBUG, "Initialized client data to peername %s, submitter %s\n", actual_data->peer_name, actual_data->current_mail.submitter);
	...
	client_send(&(clients->conns[client_slot]), "220 %s ESMTP service ready\r\n", listener_data->announce_domain);
	return 0;
}
...
int client_process(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;
	...
	if(left < 2){
		...
		client_send(client, "500 Line too long\r\n");
		client_close(client);
		return 0;
	}
		...
		switch(client->tls_mode){
			case TLS_NEGOTIATE:
				...
				if(client_data->listener->tls_mode == TLS_ONLY){
					...
					client_send(client, "220 %s ESMTPS service ready\r\n", listener_data->announce_domain);
				}
				break;
			default:
		#endif
	...
	do{
		line_length = common_next_line(client_data->recv_buffer, &(client_data->recv_offset), &bytes);
		if(line_length >= 0){
			if(line_length >= SMTP_MAX_LINE_LENGTH - 2){
				logprintf(LOG_WARNING, "Line too long, ignoring\n");
				client_send(client, "500 Line too long\r\n");
				//client_line(client, database, path_pool);
				...
				client_data->connection_score += client_line(client, database, path_pool);
				...
				if(client_data->connection_score < CMAIL_FAILSCORE_LIMIT){
					...
					client_send(client, "500 Too many failed commands\r\n");
					client_close(client);
					return 0;
				}
			}
