int smtpstate_new(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	...
	if(!strncasecmp(client_data->recv_buffer, "ehlo ", 5)){
		...
		switch(listener_data->auth_offer){
			...
			case AUTH_TLSONLY:
				...
				#endif
			case AUTH_ANY:
				client_send(client, "250-AUTH PLAIN\r\n");
				break;
		}
		#ifndef CMAIL_NO_TLS
		...
		if(client_data->listener->tls_mode == TLS_NEGOTIATE && client->tls_mode == TLS_NONE){
			client_send(client, "250-STARTTLS\r\n"); //RFC 3207
		}
		#endif
		client_send(client, "250 XYZZY\r\n"); //RFC 5321 2.2.2
		return 0;
	}
	...
	if(!strncasecmp(client_data->recv_buffer, "helo ", 5)){
		...
		client_data->state = STATE_IDLE;
		logprintf(LOG_INFO, "Client negotiates smtp\n");

		client_send(client, "250 %s ahoyhoy\r\n", listener_data->announce_domain);
		return 0;
	}

	logprintf(LOG_INFO, "Command not recognized in state NEW: %s\n", client_data->recv_buffer);
	client_send(client, "500 Unknown command\r\n");
	return -1;
}

int smtpstate_auth(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;
	...
	if(client_data->sasl_context.method != SASL_INVALID && !strcmp(client_data->recv_buffer, "*")){
		...
		client_send(client, "501 Authentication cancelled\r\n");
		return 0;
	}
	else if(client_data->sasl_context.method != SASL_INVALID){
		status = sasl_continue(&(client_data->sasl_context), client_data->recv_buffer, &challenge);
	}
		...
		if(method){
			...
			status = sasl_begin(&(client_data->sasl_context), &(client_data->sasl_user), method, parameter, &challenge);
		}
		else{
		...
		logprintf(LOG_ERROR, "Invalid state (No negotiation but data) reached in AUTH, returning to IDLE\r\n");
		client_data->state = STATE_IDLE;
		client_send(client, "500 Invalid state\r\n");
		return -1;
	}

	switch(status){
		case SASL_OK:
			logprintf(LOG_ERROR, "Invalid state (SASL_OK) reached in AUTH, returning to IDLE\r\n");
		...
		case SASL_ERROR_PROCESSING:
			...
			client_send(client, "454 Internal Error\r\n");
			client_data->state = STATE_IDLE;
			return 0;
		case SASL_ERROR_DATA:
			logprintf(LOG_ERROR, "SASL failed to parse data\r\n");
			sasl_reset_ctx(&(client_data->sasl_context), true);
			client_send(client, "501 Invalid data provided\r\n");
			client_data->state = STATE_IDLE;
			return -1;
		case SASL_UNKNOWN_METHOD:
			...
			client_send(client, "504 Unknown mechanism\r\n");
			return -1;
		case SASL_CONTINUE:
			logprintf(LOG_INFO, "Asking for SASL continuation\r\n");
			client_send(client, "334 %s\r\n", challenge ? challenge:"");
			return 0;
		case SASL_DATA_OK:
			sasl_reset_ctx(&(client_data->sasl_context), true);
			client_data->state = STATE_IDLE;
			...
			//check auth data
			if(!challenge || auth_validate(database->query_authdata, client_data->sasl_user.authenticated, challenge, &(client_data->sasl_user.authorized)) < 0){
				//login failed
				sasl_reset_user(&(client_data->sasl_user), true);
				logprintf(LOG_INFO, "Client failed to authenticate\n");
				client_send(client, "535 Authentication failed\r\n");
			...
			client_data->originating_route = route_query(database, client_data->sasl_user.authorized);

			logprintf(LOG_INFO, "Client authenticated as %s\n", client_data->sasl_user.authenticated);
			client_send(client, "235 Authenticated\r\n");
...
int smtpstate_idle(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;
	LISTENER* listener_data = (LISTENER*)client_data->listener->aux_data;

	if(!strncasecmp(client_data->recv_buffer, "noop", 4)){
		logprintf(LOG_INFO, "Client noop\n");
		client_send(client, "250 OK\r\n");
		return 0;
	}

	if(!strncasecmp(client_data->recv_buffer, "rset", 4)){
		logprintf(LOG_INFO, "Client reset\n");
		mail_reset(&(client_data->current_mail));
		client_send(client, "250 Reset OK\r\n");
		return 0;
	}
	...
	if(!strncasecmp(client_data->recv_buffer, "starttls", 8)){
		if(client->tls_mode != TLS_NONE){
			...
			client_send(client, "503 Already in TLS session\r\n");
			return 0;
		}
		...
		if(client_data->listener->tls_mode != TLS_NEGOTIATE){
			...
			client_send(client, "503 Not advertised\r\n");
		...
		client_send(client, "220 Go ahead\r\n");
	...
	if(!strncasecmp(client_data->recv_buffer, "auth ", 5)){
		switch(listener_data->auth_offer){
			case AUTH_NONE:
				logprintf(LOG_WARNING, "Client tried to auth on a non-auth listener\n");
				client_send(client, "503 Not advertised\r\n");
				return -1;
			case AUTH_TLSONLY:
				#ifndef CMAIL_NO_TLS
				if(client->tls_mode != TLS_ONLY){
				#endif
					logprintf(LOG_WARNING, "Non-TLS client tried to auth on auth-tlsonly listener\n");
					client_send(client, "504 Encryption required\r\n"); //FIXME 538 might be better, but is marked obsolete
					return -1;
				#ifndef CMAIL_NO_TLS
				}
				#endif
			case AUTH_ANY:
				if(client_data->sasl_user.authenticated){
					logprintf(LOG_WARNING, "Authenticated client tried to authenticate again\n");
					client_send(client, "503 Already authenticated\r\n");
					return -1;
				}
				//continue
				break;
	...
	if(!strncasecmp(client_data->recv_buffer, "quit", 4)){
		logprintf(LOG_INFO, "Client quit\n");
		client_send(client, "221 OK Bye\r\n");
		return client_close(client);
	}

	//this needs to be implemented as per RFC5321 3.3
	if(!strncasecmp(client_data->recv_buffer, "rcpt ", 5)){
		logprintf(LOG_INFO, "Client tried to use RCPT in IDLE\n");
		client_send(client, "503 Bad sequence of commands\r\n");
		return -1;
	}

	if(!strncasecmp(client_data->recv_buffer, "mail from:", 10)){
		if(listener_data->auth_require && !client_data->sasl_user.authenticated){
			logprintf(LOG_WARNING, "Client tried mail command without authentication on strict auth listener\n");
			client_send(client, "530 Authentication required\r\n");
			return -1;
		}

		logprintf(LOG_INFO, "Client initiates mail transaction\n");
		//extract reverse path and store it
		if(path_parse(client_data->recv_buffer + 10, &(client_data->current_mail.reverse_path)) < 0){
			client_send(client, "501 Path rejected\r\n");
			return -1;
		}

		//resolve reverse path
		switch(path_resolve(&(client_data->current_mail.reverse_path), database, client_data->sasl_user.authorized, true)){
			case 0:
				...
				if(client_data->sasl_user.authenticated && client_data->sasl_user.authorized){
					if(!client_data->originating_route.router || !strcmp(client_data->originating_route.router, "reject")){
						...
						if(!client_data->current_mail.reverse_path.route.router
							...
							client_send(client, "551 User not allowed to use this path\r\n");
							path_reset(&(client_data->current_mail.reverse_path));
							return -1;
						}
						else{
			...
			case 1:
				...
				logprintf(LOG_ERROR, "Inbound reject router applied to reverse path, please notify the developers!\n");
				break;
			default:
				//resolution failed
				logprintf(LOG_INFO, "Failed to resolve reverse path\n");
				path_reset(&(client_data->current_mail.reverse_path));
				client_send(client, "451 Path rejected\r\n");
				return 0;
		}
	...
	if(!strncasecmp(client_data->recv_buffer, "vrfy ", 5)
		...
		logprintf(LOG_WARNING, "Client tried to verify / expand an address, unsupported\n");
		client_send(client, "502 Not implemented\r\n");
		return -1;
	}

	logprintf(LOG_INFO, "Command not recognized in state IDLE: %s\n", client_data->recv_buffer);
	client_send(client, "500 Unknown command\r\n");
	return -1;
}

int smtpstate_recipients(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;
	LISTENER* listener_data = (LISTENER*)client_data->listener->aux_data;
	unsigned i;
	...
	if(!strncasecmp(client_data->recv_buffer, "rcpt to:", 8)){
		...
		if(i == SMTP_MAX_RECIPIENTS){
			...
			logprintf(LOG_INFO, "Mail exceeded recipient limit\n");
			client_send(client, "452 Too many recipients\r\n");
			return -1;
		}

		//get path from pool
		current_path = pathpool_get(path_pool);
		if(!current_path){
			logprintf(LOG_ERROR, "Failed to get path, failing recipient\n");
			client_send(client, "452 Recipients pool maxed out\r\n");
			//FIXME should state transition back to idle here?
			return -1;
		}

		if(path_parse(client_data->recv_buffer + 8, current_path) < 0){
			client_send(client, "501 Path rejected\r\n");
		...
		switch(path_resolve(current_path, database, NULL, false)){
			case 0:
		...
		if(!current_path->route.router){
			...
			if(current_path->path[current_path->delimiter_position] != '@'){
				client_send(client, "551 Non-local paths need domain part\r\n");
				pathpool_return(current_path);
				return -1;
			}
			//accept only if authenticated
			if(!client_data->sasl_user.authenticated){
				client_send(client, "551 Unknown user\r\n");
				pathpool_return(current_path);
				return -1;
			}
		}
		...
		client_send(client, "250 Accepted\r\n");
	...
	if(!strncasecmp(client_data->recv_buffer, "auth", 4)){
		logprintf(LOG_INFO, "Client tried to use AUTH in RECIPIENTS\n");
		client_send(client, "503 Bad sequence of commands\r\n");
		return -1;
	}
	...
	if(!strncasecmp(client_data->recv_buffer, "data", 4)){
		...
		if(!client_data->current_mail.forward_paths[0]){
			logprintf(LOG_INFO, "Data without recipients\n");
			client_send(client, "503 No valid recipients\r\n");
			return -1;
		}
		client_data->state = STATE_DATA;

		//write received: header
		if(mail_recvheader(&(client_data->current_mail), listener_data->announce_domain, listener_data->suppress_submitter) < 0){
			logprintf(LOG_WARNING, "Failed to write received header\n");
		}

		logprintf(LOG_INFO, "Client begins data transmission\n");

		client_send(client, "354 Go ahead\r\n");
		return 0;
	}

	logprintf(LOG_INFO, "Command not recognized in state RECIPIENTS: %s\n", client_data->recv_buffer);
	client_send(client, "500 Unknown command\r\n");
	return -1;
}

int smtpstate_data(CONNECTION* client, DATABASE* database, PATHPOOL* path_pool){
	CLIENT* client_data = (CLIENT*)client->aux_data;

	if(client_data->recv_buffer[0] == '.'){
		if(client_data->recv_buffer[1]){
			logprintf(LOG_INFO, "Data line with leading dot, fixing\n");
			//skip leading dot
			//FIXME use return value (might indicate message too long)
			if(mail_line(&(client_data->current_mail), client_data->recv_buffer + 1) < 0){
				logprintf(LOG_WARNING, "Failed to store mail data line\n");
			}
			return 0;
		}
		else{
			...
			if(client_data->current_mail.data_max && client_data->current_mail.data_offset > client_data->current_mail.data_max){
				logprintf(LOG_WARNING, "End of mail, data section exceeded size limitation, rejecting\n");
				client_send(client, "552 Size limit exceeded\r\n");
			}
			//check for too many hops
			else if(client_data->current_mail.hop_count > CMAIL_MAX_HOPS){
				logprintf(LOG_WARNING, "Mail exceeded maximum hop count of %d\n", CMAIL_MAX_HOPS);
				client_send(client, "554 Too many hops\r\n");
			}
			else{
				logprintf(LOG_INFO, "End of mail data, routing\n");

				//TODO call plugins here
				if(!client_data->sasl_user.authenticated){
					switch(mail_route(&(client_data->current_mail), database)){
						case 250:
							logprintf(LOG_INFO, "Incoming mail accepted from %s\n", client_data->peer_name);
							client_send(client, "250 OK\r\n");
							break;
						case 400:
							logprintf(LOG_INFO, "Temporary routing failure, deferring message\n");
							client_send(client, "451 Temporary failure, please try again later. If the error persists, contact the administrator.\r\n");
							break;
						default:
							logprintf(LOG_WARNING, "Mail not routed, rejecting\n");
							client_send(client, "554 Rejected\r\n");
							break;
					}
				}
				else{
					switch(mail_originate(client_data->sasl_user.authorized, &(client_data->current_mail), client_data->originating_route, database)){
						case 250:
							logprintf(LOG_INFO, "Originating mail accepted for user %s (auth %s) from %s\n", client_data->sasl_user.authorized, client_data->sasl_user.authenticated, client_data->peer_name);
							client_send(client, "250 OK\r\n");
							break;
						case 400:
							logprintf(LOG_INFO, "Temporary routing failure, deferring message\n");
							...
							break;
						default:
							logprintf(LOG_WARNING, "Originated mail could not be routed, rejecting\n");
							client_send(client, "554 Rejected\r\n");
							break;
					}
				}
		...
		if(client_data->current_mail.header_offset == 0){
			...
			if(client_data->recv_buffer[0] == 0){
				...
				logprintf(LOG_DEBUG, "End of header detected at %d\n", client_data->current_mail.header_offset);
			}
			//count Received: headers
			else if(!strncmp(client_data->recv_buffer, "Received: ", 10)){
				client_data->current_mail.hop_count++;
				logprintf(LOG_DEBUG, "Mail is now at %d hops\n", client_data->current_mail.hop_count);
			}
		}

		//FIXME use return value (might indicate message too long)
		if(mail_line(&(client_data->current_mail), client_data->recv_buffer) < 0){
			logprintf(LOG_WARNING, "Failed to store mail data line\n");
		}
		return 0;
	}
