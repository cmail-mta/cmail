int client_line(CONNECTION* client, DATABASE* database){
	...
	switch(client_data->state){
		case STATE_AUTH:
			return state_authorization(client, database);
		case STATE_TRANSACTION:
			return state_transaction(client, database);
		case STATE_UPDATE:
			return state_update(client, database);
	}
...
int client_accept(CONNECTION* listener, CONNPOOL* clients){
	...
	*actual_data = empty_data;
	...
	client_send(&(clients->conns[client_slot]), "+OK %s POP3 ready\r\n", listener_data->announce_domain);
	return 0;
}
...
int client_close(CONNECTION* client, DATABASE* database){
	...
	if(client_data->auth.auth_ok && client_data->auth.user.authenticated){
		maildrop_release(database, &(client_data->maildrop), client_data->auth.user.authorized);
	}
...
int client_process(CONNECTION* client, DATABASE* database){
	CLIENT* client_data = (CLIENT*)client->aux_data;
	...
	if(left < 2){
		...
		client_send(client, "-ERR Line too long\r\n");
		client_close(client, database);
		return 0;
	}
		...
		switch(client->tls_mode){
			case TLS_NEGOTIATE:
				...
				if(client_data->listener->tls_mode == TLS_ONLY){
					...
					client_send(client, "+OK %s POP3 ready\r\n", listener_data->announce_domain);
				}
				break;
			default:
		#endif
	...
	do{
		...
		if(line_length >= 0){
			if(line_length >= POP_MAX_LINE_LENGTH-2){
				...
				client_data->connection_score += client_line(client, database);
				...
				if(client_data->connection_score < CMAIL_FAILSCORE_LIMIT){
					...
					client_send(client, "-ERR Too many failed commands\r\n");
					client_close(client, database);
					return 0;
				}
			}
