int logic_generate_bounces(DATABASE* database, MTA_SETTINGS settings){
	...
	logprintf(LOG_INFO, "Entering bounce handling logic\n");
	...
	if(sqlite3_bind_int(database->query_bounce_candidates, 1, settings.mail_retries) != SQLITE_OK){
		logprintf(LOG_ERROR, "Failed to bind retry amount parameter %d: %s\n", settings.mail_retries, sqlite3_errmsg(database->conn));
		return -1;
	}
		...
		switch(status){
			case SQLITE_ROW:
				...
				if(!sqlite3_column_text(database->query_bounce_candidates, 1)){
					logprintf(LOG_WARNING, "Ignoring bounce message generated for a bounce message\n");
					mail_delete(database, sqlite3_column_int(database->query_bounce_candidates, 0));
					continue;
				}

				logprintf(LOG_DEBUG, "Bouncing message %d from %s retries %d fatality %d\n", sqlite3_column_int(database->query_bounce_candidates, 0), sqlite3_column_text(database->query_bounce_candidates, 1), sqlite3_column_int(database->query_bounce_candidates, 4), sqlite3_column_int(database->query_bounce_candidates, 5));
				...
				if(!bounce_message){
					logprintf(LOG_ERROR, "Failed to create bounce message base\n");
					break;
				}
				...
				if(sqlite3_bind_int(database->query_bounce_reasons, 1, sqlite3_column_int(database->query_bounce_candidates, 0)) != SQLITE_OK){
					logprintf(LOG_ERROR, "Failed to bind bounce reason query parameter: %s\n", sqlite3_errmsg(database->conn));
				}
				...
				do{
					...
					switch(status){
						case SQLITE_ROW:
							...
							if(!bounce_message){
								logprintf(LOG_ERROR, "Failed to append bounce reason to bounce message\r\n");
								break;
							}
							break;
						case SQLITE_DONE:
							break;
						default:
							logprintf(LOG_ERROR, "Unhandled response to bounce reason query: %s\n", sqlite3_errmsg(database->conn));
							break;
					}
				}
				...
				if(!bounce_message){
					logprintf(LOG_ERROR, "Failed to append to bounce message\n");
					break;
				}
				...
				if(sqlite3_bind_text(database->insert_bounce, 2, bounce_message, -1, SQLITE_STATIC) != SQLITE_OK
					...
					logprintf(LOG_ERROR, "Failed to bind bounce insertion parameter: %s\n", sqlite3_errmsg(database->conn));
				}
				else{
					...
					do{
						...
						if(sqlite3_step(database->insert_bounce) != SQLITE_DONE){
							logprintf(LOG_ERROR, "Failed to insert bounce message\n");
							rv = -1;
						}
						...
						if(settings.bounce_to && settings.bounce_to[i]){
							if(sqlite3_bind_text(database->insert_bounce, 1, settings.bounce_to[i], -1, SQLITE_STATIC) != SQLITE_OK){
								logprintf(LOG_ERROR, "Failed to bind additional bounce recipient: %s\n", sqlite3_errmsg(database->conn));
								rv = -1;
							}
						}
					}
				...
				break;
			case SQLITE_DONE:
				logprintf(LOG_INFO, "Generated %d bounce messages\n", bounces);
				break;
			default:
				logprintf(LOG_WARNING, "Unhandled database response in bounce generation: %s\n", sqlite3_errmsg(database->conn));
				rv = -1;
				break;
		}
	}
...
int logic_handle_transaction(DATABASE* database, CONNECTION* conn, MAIL* transaction){
	int delivered_mails = 0;
	unsigned i;
	CONNDATA* conn_data = (CONNDATA*) conn->aux_data;
	bool fail_permanently = false;

	if(mail_dispatch(database, transaction, conn) < 0){
		logprintf(LOG_WARNING, "Dispatch function reported permanent failure\n");
	...
	for(i = 0; i < transaction->recipients; i++){
		switch(transaction->rcpt[i].status){
			case RCPT_OK:
				...
				if(mail_delete(database, transaction->rcpt[i].dbid) < 0){
					logprintf(LOG_WARNING, "Failed to delete delivered mail id %d\n", transaction->rcpt[i].dbid);
				}
			...
			case RCPT_FAIL_PERMANENT:
				...
				break;
			case RCPT_READY:
				logprintf(LOG_WARNING, "Recipient %d not touched by dispatch loop\n", i);
				//this happens e.g. if the peer rejects the MAIL command
				mail_failure(database, transaction->rcpt[i].dbid, conn_data->reply.response_text, fail_permanently);
				break;
		}
	}

	logprintf(LOG_INFO, "Handled %d mails in transaction\n", delivered_mails);
	return delivered_mails;
}

int logic_handle_remote(DATABASE* database, MTA_SETTINGS settings, REMOTE remote){
	...
	if(!remote.host || strlen(remote.host) < 2){
		logprintf(LOG_ERROR, "No valid hostname provided\n");
		return -1;
	}

	logprintf(LOG_INFO, "Entering mail remote handling for host %s in mode %s\n", remote.host, (remote.mode == DELIVER_DOMAIN) ? "domain":"handoff");

	//resolve host for MX records
	resolver_remote = remote.host;
	if(remote.mode == DELIVER_DOMAIN){
		status = adns_init(&resolver, adns_if_none, log_output(NULL));
		if(status != 0){
			logprintf(LOG_ERROR, "Failed to initialize adns: %s\n", strerror(status));
			return -1;
		}

		status = adns_synchronous(resolver, remote.host, adns_r_mx, adns_qf_cname_loose, &resolver_answer);
		if(status != 0){
			logprintf(LOG_ERROR, "Failed to run query: %s\n", strerror(status));
			return -1;
		}

		logprintf(LOG_DEBUG, "%d records in DNS response\n", resolver_answer->nrrs);

		if(resolver_answer->status == adns_s_ok && resolver_answer->nrrs <= 0){
			logprintf(LOG_ERROR, "No MX records for domain %s found, falling back to HANDOFF strategy\n", remote.host);
			...
			if(resolver_answer->status == adns_s_nxdomain){
				logprintf(LOG_ERROR, "DNS query returned an irrecoverable error, failing this remote permanently\n");
			}
			//retry
			//	the rest
			else{
				logprintf(LOG_ERROR, "DNS query returned a recoverable error, retrying this remote in the next interval\n");
				free(resolver_answer);
				adns_finish(resolver);
				return -1;
			}
	...
	if(sqlite3_bind_int(tx_statement, 1, settings.retry_interval) != SQLITE_OK){
		logprintf(LOG_ERROR, "Failed to bind timeout parameter\n");
	...
	if(sqlite3_bind_text(tx_statement, 2, remote.remotespec, -1, SQLITE_STATIC) != SQLITE_OK){
		logprintf(LOG_ERROR, "Failed to bind host parameter\n");
		//FIXME returning -1 here retries in the next interval, leaking adns_*
		return -1;
	}
		...
		switch(status){
			case SQLITE_ROW:
				if(tx_active >= tx_allocated){
					...
					if(!mails){
						logprintf(LOG_ERROR, "Failed to allocate memory for mail transaction array\n");
						//FIXME returning -1 here retries in the next interval, leaking adns_*
						return -1;
					}
				...
				//read transaction
				if(mail_dbread(mails + tx_active, tx_statement) < 0){
					logprintf(LOG_ERROR, "Failed to read transaction %s, database status %s\n", (char*)sqlite3_column_text(tx_statement, 0), sqlite3_errmsg(database->conn));
				}

				tx_active++;
				break;
			case SQLITE_DONE:
				logprintf(LOG_INFO, "Connection handles %d transactions\n", tx_active);
				break;
			default:
				logprintf(LOG_WARNING, "Unhandled response from mail transaction query: %s\n", sqlite3_errmsg(database->conn));
				break;
		}
	}
	while(status == SQLITE_ROW);
	...
	for(current_mx = 0; current_mx < mx_count; current_mx++){
		...
		logprintf(LOG_INFO, "Trying to connect to MX %d: %s\n", current_mx, resolver_remote);
		...
		for(port = 0; settings.port_list[port].port; port++){
			...
			if(remote.mode == DELIVER_HANDOFF && remote.forced_port.port){
				...
				logprintf(LOG_INFO, "Using forced port for handoff remote\n");
				current_port = remote.forced_port;
			}

			#ifndef CMAIL_NO_TLS
			logprintf(LOG_INFO, "Trying port %d TLS mode %s\n", current_port.port, tls_modestring(current_port.tls_mode));
			#else
			...
			if(conn.fd > 0){
				//negotiate smtp
				if(smtp_negotiate(settings, resolver_remote, &conn, current_port, remote.remote_auth) < 0){
					logprintf(LOG_INFO, "Failed to negotiate required protocol level, trying next\n");
				...
				for(c = 0; c < tx_active; c++){
					status = logic_handle_transaction(database, &conn, mails + c);
					if(status < 0){
						logprintf(LOG_WARNING, "Mail transaction failed, continuing\n");
					}
					else{
				...
				logprintf(LOG_INFO, "Delivered %d mails in %d transactions for this remote\n", delivered_mails, tx_active);
	...
	logprintf(LOG_INFO, "Finished delivery handling of host %s after trying %d of %d MXes\n", remote.host, current_mx, mx_count);
	...
	if(delivered_mails < 0){
		...
		if(remote.mode == DELIVER_DOMAIN && 
			...
			logprintf(LOG_WARNING, "Invalid remotespec %s, permanently failing\n", remote.host);
			failure_reason = "Failed to resolve recipient domain";
			fail_permanently = true;
		}
		else if(remote.mode == DELIVER_HANDOFF){
			logprintf(LOG_WARNING, "Connection or TLS negotiation failed with handoff remote %s\n", remote.host);
			failure_reason = "Failed to connect or negotiate with handoff remote";
		}
		else{
			logprintf(LOG_WARNING, "Could not reach any MX for %s, failing temporarily\n", remote.host);
		}
		
		//insert failure reasons
		for(i = 0; i < tx_active; i++){
			for(c = 0; c < mails[i].recipients; c++){
				mail_failure(database, mails[i].rcpt[c].dbid, failure_reason, fail_permanently);
			}
		}
	}
	...
	connection_reset(&conn, true);
	logprintf(LOG_INFO, "Mail delivery for %s done\n", remote.host);
	return delivered_mails;
}

int logic_loop_hosts(DATABASE* database, MTA_SETTINGS settings){
	int status;
	unsigned u;
	...
	logprintf(LOG_INFO, "Entering core loop\n");

	do{
		mails_delivered = 0;
		remotes_active = remotes_fetch(database, settings, &remotes, &remotes_allocated);
		if(remotes_active < 0){
			//FIXME how to handle these errors
			logprintf(LOG_ERROR, "Remote fetching failed, error handling unclear - continuing\n");
			remotes_active = 0;
		}
		logprintf(LOG_INFO, "Interval contains %d remote(s)\n", remotes_active);

		//deliver all remotes
		for(u = 0; u < remotes_active; u++){
			logprintf(LOG_INFO, "Starting delivery for %s in mode %s\n", remotes[u].host, (remotes[u].mode == DELIVER_DOMAIN) ? "domain":"handoff");
			if(remotes[u].forced_port.port){
				#ifndef CMAIL_NO_TLS
				logprintf(LOG_INFO, "Remote uses forced port %d with TLS mode %s\n", remotes[u].forced_port.port, tls_modestring(remotes[u].forced_port.tls_mode));
				#else
				logprintf(LOG_INFO, "Remote uses forced port %d\n", remotes[u].forced_port.port);
				#endif
			}
			if(remotes[u].remote_auth){
				logprintf(LOG_INFO, "Remote uses remote authentication\n");
				logprintf(LOG_DEBUG, "Remote authentication data %s\n", remotes[u].remote_auth);
			}

			//TODO implement multi-threading here
			status = logic_handle_remote(database, settings, remotes[u]);

			if(status < 0){
				logprintf(LOG_WARNING, "Delivery procedure returned an error\n");
			}
			else{
		...
		bounces_generated = logic_generate_bounces(database, settings);
		if(bounces_generated < 0){
			logprintf(LOG_WARNING, "Bounce generation reported an error\n");
			bounces_generated = 0;
		}

		logprintf(LOG_INFO, "Core interval done, delivered %d mails, generated %d bounces\n", mails_delivered, bounces_generated);
	...
	logprintf(LOG_INFO, "Core loop exiting cleanly\n");
	return 0;
}
